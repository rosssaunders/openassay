...
fn test_openassay_supported_features() {
    ...
    let mut run_test = |name: &str, sql: &str, should_succeed: bool| {
        ...
        if should_succeed && !has_error {
            ...
            test_results.push((
                name.to_string(),
                "FAIL: Expected error but succeeded".to_string(),
            ));
        }
    };
    ...
    run_test(
        "create_table",
        "CREATE TABLE test_users (id INTEGER, name TEXT, email TEXT)",
        true,
    );
    ...
    run_test("select_from_table", "SELECT * FROM test_users", true);
    run_test(
        "select_with_where",
    ...
    run_test(
        "update_record",
        "UPDATE test_users SET email = 'alice.updated@example.com' WHERE id = 1",
        true,
    );
    run_test("delete_record", "DELETE FROM test_users WHERE id = 2", true);

    // 6. JOINs
    run_test(
        "create_orders_table",
        "CREATE TABLE test_orders (id INTEGER, user_id INTEGER, amount NUMERIC)",
        true,
    );
    run_test(
        "insert_order",
        "INSERT INTO test_orders VALUES (100, 1, 49.99)",
        true,
    );
    run_test(
        "inner_join",
        "SELECT u.name, o.amount FROM test_users u JOIN test_orders o ON u.id = o.user_id",
        true,
    );
    run_test(
        "left_join",
    ...
    run_test(
        "subquery_exists",
        "SELECT name FROM test_users WHERE EXISTS (SELECT 1 FROM test_orders WHERE user_id = test_users.id)",
        true,
    );
    run_test(
        "subquery_in",
        "SELECT name FROM test_users WHERE id IN (SELECT user_id FROM test_orders)",
        true,
    );

    // 8. Aggregates and GROUP BY
    run_test("count_aggregate", "SELECT COUNT(*) FROM test_users", true);
    run_test(
        "group_by",
        "SELECT user_id, COUNT(*) FROM test_orders GROUP BY user_id",
        true,
    );
    run_test(
        "having_clause",
        "SELECT user_id, COUNT(*) FROM test_orders GROUP BY user_id HAVING COUNT(*) > 0",
        true,
    );

    // 9. DISTINCT
    run_test("distinct", "SELECT DISTINCT user_id FROM test_orders", true);

    // 10. CASE expressions
    run_test(
        "case_expression",
        "SELECT name, CASE WHEN id = 1 THEN 'first' ELSE 'other' END AS category FROM test_users",
        true,
    );
    ...
    run_test(
        "like_pattern",
        "SELECT name FROM test_users WHERE name LIKE 'A%'",
        true,
    );
    ...
    run_test(
        "cast_operation",
        "SELECT CAST(123 AS TEXT) AS text_number",
        true,
    );
    run_test("double_colon_cast", "SELECT 123::TEXT AS text_number", true);

    // 13. Window functions
    run_test(
        "window_row_number",
        "SELECT name, ROW_NUMBER() OVER (ORDER BY id) FROM test_users",
        true,
    );
    ...
    run_test(
        "simple_cte",
        "WITH user_summary AS (SELECT COUNT(*) as total FROM test_users) SELECT total FROM user_summary",
        true,
    );
    ...
    run_test(
        "union",
        "SELECT name FROM test_users UNION SELECT 'Additional' AS name",
        true,
    );
    ...
    run_test(
        "string_functions",
        "SELECT LENGTH(name), UPPER(name) FROM test_users",
        true,
    );
    ...
    run_test(
        "math_functions",
        "SELECT ABS(-42), SQRT(16), ROUND(3.14159, 2)",
        true,
    );
    ...
    // 20. EXPLAIN
    run_test("explain_query", "EXPLAIN SELECT * FROM test_users", true);

    // 21. CREATE VIEW
    run_test(
        "create_view",
        "CREATE VIEW user_view AS SELECT name, email FROM test_users",
        true,
    );
    ...
    // 24. Sequences
    run_test("create_sequence", "CREATE SEQUENCE test_seq", true);
    run_test("nextval_sequence", "SELECT nextval('test_seq')", true);
